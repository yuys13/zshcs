...
if [[ ! -d $ZSHCS_CACHE_DIR ]]; then
  mkdir -p -- "$ZSHCS_CACHE_DIR" || { echo "error: failed to create cache dir: $ZSHCS_CACHE_DIR" >&2; exit 1; }
fi

# Load completion system
autoload -U compinit
compinit -C -d "$ZSHCS_CACHE_DIR/compdump"
...
compadd () {
    ...
    for i in {1..$#__hits}; do
        ...
        # Use tab separator as agreed in architecture
